...
router.route('/findrating').get(async(req, res, next)=>{
    try
    {
        let query = req.query.id
        let response=await  User.findById(query) 
        if(response && response.rating && response.rating.avgRating)
        {
            res.json(response.rating.avgRating) 
        }
...
router.route('/setrating').put(async (req, res, next)=>{
    try
    {
        let id = req.body.id, newRating=req.body.rating
        
        let user = await User.findById(id)
        if(user.rating && user.rating.ratingTime)
        {
            let ratingTime=user.rating.ratingTime, rating=user.rating.avgRating
            newRating = (rating*ratingTime+newRating)/(ratingTime+1)
            ratingTime++
            let curRating={ratingTime:ratingTime, avgRating:newRating}
           const resp= await User.findByIdAndUpdate(id, {rating:curRating})
            res.json('User updated!')
        }
        else
        {
