...
let Qoute = require('../schemas/qoute.schema');
...
router.route('/add').post(async (req, res)=>{
    try
    {
        let qoute = req.body
                ...
                const newQoute = new Qoute({qouteBy:[],
                    customerInfo:{userName:qoute.customerInfo.username, id:qoute.customerInfo._id,
                         phone:qoute.phone, email:qoute.email }, status:'Pending', problem:qoute.problem,
                          message:qoute.message, rate:0, isDeleted:false, rating:'', 

                })
                const resp= await newQoute.save()
                res.json(resp)
...
router.route('/find/?').get(async(req, res)=>{
    try{
        const query = {"customerInfo.userName":req.query.username, status:req.query.status}
        const resp = await Qoute.find(query).sort({createdAt:-1})
...
router.route('/delete/?').delete(async(req, res)=>{
    try{
        const resp = await Qoute.findByIdAndDelete(req.query._id)
        res.json(resp)
    }catch(err)
    {
        console.log(err)
...
router.route('/addqoute').put(async(req, res)=>{
    try
    {
        let data = req.body
        let id = data._id
        const resp = await Qoute.findByIdAndUpdate(id, data)
        res.json(resp)
    }
    catch(err)
    {
...
router.route('/completejob').put(async(req, res, next)=>{
    try
    {
        let data = {...req.body, status:'Complete'}
        let id = data._id
        const resp = await Qoute.findByIdAndUpdate(id, data)
        res.json(resp)
    }
...
//                             wordsArray.map(word=>
//                                 queryArray.({[objName]:{"$regex":word, "$options": "i"}}) 
//                             )
...
//           saleAmount.(prices[i].gTotal)
//           let curProfit=prices[i].gTotal-prices[i].pAmount
//           profit.(curProfit)
//           purchaseAmount.(prices[i].pAmount)
//           cashReceived.(prices[i].amountReceived)
//           dates.(`${prices[i]._id.day}/${prices[i]._id.month}/${prices[i]._id.year}`)
//         }
...
//                             purchaseAmount.(0)
//                             profit.(0)
//                             dates.(`${missingMonth}/${prices[i]._id.year}`)
//                         } 
//                     }
...
//                     profit.(curProfit)
//                     purchaseAmount.(prices[i].pAmount)
//                     dates.(`${prices[i]._id.month}/${prices[i]._id.year}`)
...
//                         orderItems.({itemName:resp[i].orders[j].itemName, count:1, status:resp[i].paymentStatus})
//                    }
//                    else
//                    {
...
//                    {
//                     if(resp[i].orders[j].itemName.trim() !=='Teflon Tape' && resp[i].orders[j].itemName.trim() !=='Daga')
//                             returnItems.({itemName:resp[i].orders[j].itemName, count:1, status:resp[i].paymentStatus})
//                    }
//                    else
//                    {
...
//                 {
//                     orderFreq.(orderItems[i].count)
//                     itemNames.(orderItems[i].itemName)
//                 }
//             for(let i=0;i<returnItems.length;i++)
//                 {
//                     returnFreq.(returnItems[i].count)
//                     returnItemsName.(returnItems[i].itemName)
//                 }
